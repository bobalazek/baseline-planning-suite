...
> Source: _Baseline Planning Suite, Senior Frontend Engineer case study_ (innoscripta SE).
...
2. The same people are shared across projects, so _capacity_ only means something when every
...
(`@repo/contracts`), handed to them by the shell at runtime.
...
The month's cost is _exactly linear in hours_. This is why the published pricing contract can be a
...
| D3  | Data layer & persistence  | **Two domain APIs, each with its own file-backed store behind a `DocumentStore` port** | [ADR-0003](decisions/0003-data-layer-and-persistence.md)  |
